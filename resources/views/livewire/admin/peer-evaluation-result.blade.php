<div>
    ...
    @if(in_array($form['action'], ['view']))
        <div class="w-100 block md:flex justify-between items-center gap-2 mt-5">
            <div>
                <a wire:navigate href="{{route('admin.programs.peer-evaluation', ['id' => $form['id']])}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        </div>
        ...
        <div class="bg-white border shadow rounded-lg mt-8 p-4 sm:p-8">
            <div class="mb-4">
                ...
                <div class="bg-white p-4 border shadow-lg rounded-b text-sm font-medium">
                    <ul>
                        ...
                        <li>Branch: {{$view['all_templates']->departments->branches->name}}</li>
                        <li>Department: {{$view['all_templates']->departments->name}}</li>
                        {{-- <li>Academic Year:
                            @dd($view['faculty']['templates'][0]['curriculum_template'][0])
                            ...
                            {{-- papalitan, gagawing school year
                        </li> --}}
                    </ul>
                </div>
            </div>
            ...
            <div class="relative my-8 w-100 sm:flex gap-3">
                ...
                @foreach ($view['all_templates']->templates as $key => $template)
                    @if($form['tab'] == $key+1)
                        <a wire:navigate href="{{ route('admin.programs.peer-evaluation', ['id' => $form['id'], 'action' => 'view', 'faculty' => $template->faculty_id, 'template' => $template->template_id, 'department' => $template->curriculum_template[0]->department_id, 'tab' => $key + 1])}}" class="px-3 border bg-sky-600 border-sky-600 py-3 text-white font-bold">
                            {{ $template->curriculum_template[0]->departments->name}} - {{ $template->curriculum_template[0]->departments->branches->name}}
                        </a>
                    @else
                        <a wire:navigate href="{{ route('admin.programs.peer-evaluation', ['id' => $form['id'], 'action' => 'view', 'faculty' => $template->faculty_id, 'template' => $template->template_id, 'department' => $template->curriculum_template[0]->department_id, 'tab' => $key + 1])}}" class="px-3 border border-sky-600 py-3 font-medium">
                            {{ $template->curriculum_template[0]->departments->name}} - {{ $template->curriculum_template[0]->departments->branches->name}}
                        </a>
                    @endif
                @endforeach
            ...
            <div class="relative my-8 w-100 sm:flex gap-3 justify-between">
                <div class="flex">
                    <a href="{{route('admin.programs.results', ['id' => $form['id'], 'action' => 'delete', 'faculty' => $form['faculty'], 'template' => $form['template'], 'department' => $form['department'], 'tab' => $form['tab']])}}" class="flex items-center gap-1 bg-red-500 text-white py-2 px-6 text-sm font-bold rounded-md w-full sm:w-100 justify-center mb-2">
                        Delete Responses
                    </a>
                </div>
                                        ...
                                        <td class="px-6 py-4 text-start">
                                            <div class="w-100 flex items-center gap-8 uppercase text-xs font-medium">
                                                ...
                                                <div>
                                                    ...
                                                    <div class = 'lowercase'>
                                                        {{-- display for sentiment analysis --}}
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-6 xl:col-span-6 2xl:col-span-4" wire:ignore>
                            <div class="relative overflow-hidden shadow rounded-lg mt-2 h-100 p-4">
                                <div class="flex items-center p-4 mb-4 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800" role="alert">
                                    <div>
                                        Percentage of faculties who have provided responses, as opposed to those who haven't responded, with consideration given to the entire pool of respondents.
                                    </div>
                                </div>
    ...
    @elseif(in_array($form['action'], ['delete']))
        <div class="w-100 block md:flex justify-between items-center gap-2 mt-5">
            <div>
                <a wire:navigate href="{{route('admin.programs.peer-evaluation', ['id' => $form['id'], 'action' => 'view', 'faculty' => $form['faculty'], 'template' => $form['template'], 'department' => $form['department']])}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
        </div>
        <div class="mt-10">
    ...
    @else
        <div class="w-100 block md:flex justify-between items-center gap-2 mt-5">
            <div>
                <a wire:navigate href="{{route('admin.programs.peer-reviews')}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div>
                                        ...
                                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 w-full" aria-labelledby="dropdownDefaultButton">
                                            <li class="text-xs whitespace-nowrap">
                                                ...
                                                @if(!empty($collection['templates']->toArray()))
                                                    <a wire:navigate href="{{route('admin.programs.peer-evaluation', ['id' => $form['id'],'template' => $collection->templates[0]->curriculum_template[0]->id, 'action' => 'view', 'faculty' => $collection->id, 'subject' => $collection->templates[0]->curriculum_template[0]->subject_id, 'department'=>$collection->department_id , 'tab' => 1])}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                        View Results
                                                    </a>
                                                @else
