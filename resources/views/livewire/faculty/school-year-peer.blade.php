            ...
            <div wire:poll class="col-span-12 md:col-span-4 bg-slate-100 shadow-lg rounded-lg text-dark relative overflow-hidden">
                <div class="max-w-sms border h-[300px] border-gray-200 rounded-lg shadow bg-slate-800 dark:border-gray-700 relative">
                    <div class="p-5 absolute bottom-2 left-0 w-full">
                        ...
                        <p class="text-sm text-white font-bold line-clamp-2">{{$collection->start_year . '-' . $collection->end_year . ' ('.to_ordinal($collection->semester, 'semester').')'}}</p>
                        <div class="mt-3">
                        ...
                        @if ($collection->status == 1)
                            <div class="mt-3 flex justify-end">
                                {{-- ITO YUNG PROBLEM --}}
                                <a wire:navigate href="{{route('faculty.peer-faculty', ['evaluate' => $collection->id, 'semester' => $collection->semester])}}" class=" bg-blue-100 text-blue-800 p-2 px-4 text-sm font-bold rounded-lg">Open</a>
                            </div>
                        @else
                            <div class="mt-3 flex justify-end">
