<div>
    <h1 class="text-3xl font-semibold">Start Peer Evaluation</h1>
    <p class="text-sm font-medium mt-1 text-slate-900 w-[75%]">Peer evaluation aims to promote professional development through constructive feedback, enhance teaching practices, foster collaboration, and share diverse perspectives, ultimately improving the quality of education provided to students.</p>
    ...
    <div class="bg-white border shadow rounded-lg mt-5 p-5">
        @if (session()->has('response') && session('response')['step'] == 1)
            <h4 class="text-2xl font-bold">Faculty Name &amp; Department</h4>
            <p class="my-2 text-sm font-medium">Note: Please ensure that faculty evaluations are conducted in alignment with established guidelines and timelines. This evaluation will be essential for ongoing <span class="uppercase font-bold underline">professional development</span> and will contribute to your performance review process..</p>
            <form wire:submit="move(2)" class="mt-10">
                    ...
                    @foreach($form[$form['action']]['data'] as $key => $item)
                        ...
                        @elseif(in_array($item['type'], ['select']))
                            <div class="{{$item['css']}}" >
                               ...
                               {{-- wire:model="{{$key}}" --}}
                               wire:model="{{$item['options']['data']['id']}}";
                                {{($item['disabled']) ? 'disabled' : ''}}>
                                @if(count($item['options']['data']) > 0)
                                    @if($item['options']['is_from_db'])
                                        <option value="{{$item['options']['data']['id']}}" {{$item['options']['data']['name'] ? 'selected' : ''}}> {{ucwords($item['options']['data']['name'])}}</option>
                                    @else
                ...
                <label class="text-gray-700 mt-2 font-bold" style="font-size: 14px">Note: Make sure the faculty selected is <span class="uppercase underline">appropriate</span>.</label>
        ...
        @elseif(session()->has('response') && session('response')['step'] == 2)
            <h4 class="text-2xl font-bold">Peer Evaluation Form</h4>
        ...
        @elseif(session()->has('response') && session('response')['step'] == 4)
            <div class="px-5">
                ...
                <hr class="my-3">
                <p class="text-sm text-slate-900 font-medium">Thank you for completing the peer evaluation. Your feedback is valuable in enhancing the teaching and learning experience. Your participation contributes to the continuous improvement of our educational environment. We appreciate your time and thoughtful responses.</p>
                <p class="mt-3 text-sm text-slate-900 font-medium">Please note: The QR code provided below serves as a means to trace your response. It can be utilized as evidence of your submission {{--,a prerequisite for clearance requirements.--}} and participation.</p>
                <div class="mt-5">
                ...
                <div class="sm:flex sm:justify-between mt-10">
                    ...
                    <button wire:navigate
                        href="{{route('faculty.peer-faculty', ['evaluate' => $evaluate, 'semester' => $semester])}}"
