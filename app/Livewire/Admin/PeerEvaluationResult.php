...
class PeerEvaluationResult extends Component
{
    ...
    public function delete_responses() {
        ...
        PeerResponseModel::where('evaluation_id', $eval_id)
            ->where('template_id', $temp_id)
            ->where('faculty_id', $fac_id)
            ->delete();

        sleep(1);
        return redirect()->route('admin.programs.peer-evaluation', ['id' => $eval_id, 'action' => 'view', 'faculty' => $fac_id, 'template' => $temp_id, 'subject' => $subj_id, 'tab' => $tab]);
    }
    ...
    public function result_view() {
        ...
        if($action == 'view') {
            ...
            if($questionnaire->count() != 0) {
                ...
                $responses = PeerResponseModel::with('faculty', 'items.questionnaire.criteria')
                ...
                foreach ($responses as $response) {
                    ...
                    $faculty_name = $response['faculty']['firstname']. ' ' . $response['faculty']['lastname'];
                ...
                // 
                $comments[] = [
                    'commented_by' => $this->applyCensored($faculty_name),
                ...
                $view = [
                    ...
                    'all_templates' => FacultyModel::with(
                                'templates.curriculum_template.departments.branches'
                            )
                            ->where('id', $faculty)
                            ->get()[0],
                ...
                return redirect()->route('admin.programs.peer-evaluation', ['id' => $this->form['id']]);
            }
        }
    }
    ...
    public function respondents() {
        $total_respondents = FacultyModel::with('templates.curriculum_template.departments.branches')
        ->has('templates')
        ->wherenot('id', $this->form['faculty']) 
        ->get()->count();

        $respondents = PeerResponseModel::where('evaluation_id', $this->form['id'])
            ->where('template_id', $this->form['template'])
            ->where('faculty_id', $this->form['faculty'])
            ->get()->count();
            ...
            foreach ($faculty_template as $template) {
                ...
                foreach ($responses as $response) {
                    foreach ($response->items as $item) {
                        ...
                        $faculty_name = $response->students->firstname . ' ' . $response->students->lastname;
                        $comments[] = [
                            'commented_by' => $this->applyCensored($faculty_name),
                            'comment' => $response->comment
                        ];
                ...
                'faculty' => FacultyModel::with([
                    'templates' => function($query) use ($template_id, $subject) {
                        $query->where('template_id', $template_id)
                            ->whereHas('curriculum_template.subjects', function($query) use ($subject) {
                                $query->where('department_id', $subject);
                            });
                    },
                    'templates.curriculum_template.subjects.courses.departments.branches'
                ])
    ...
    public function render() {
        ...
        foreach($dirty_departments as $department) {
            $branchkey = $department->branches->id;
            if(!isset($departments[$branchkey])) {
                $departments[$branchkey] = [
                    'id' => $branchkey,
                ...
                $departments[$branchkey]['departments'][] = [
                    'id' => $department->id,
        ...
        $faculty = FacultyModel::with(['departments.branches'])
            ->when(strlen($this->search['type']) >= 1, function ($sQuery) {
                $sQuery->where(function($query) {
                    ...
                    ->orWhere('lastname', 'like', '%' . $this->search['type'] . '%')
                    ->orWhere('middlename', 'like', '%' . $this->search['type'] . '%');
                });
            ...
            ->when($this->search['select'] != '', function ($query) {
                $query->whereHas('departments.branches', function ($subQuery) {
                    $subQuery->where('department_id', $this->search['select']);
                });
            })
        ...
        return view('livewire.admin.peer-evaluation-result', compact('data'));
    }
