...
class Evaluate extends Component
{
    ...
    public function step02($step) {
        ...
        //     'end_time' => 'Scheduled end time'
         ...
         $rules = [
             'faculty_id' => 'exists:afears_faculty,id|integer',
        //     'start_time' => 'required',
        //     'end_time' => 'required',
        ];
    ...
    public function step03($step) {
        ...
        $data = PeerQuestionnaireItemModel::where('questionnaire_id', $questionnaire_id)->get();
        ...
        if(!$data['faculty']['is_preview']) {
            if (array_key_exists('step', $data) && array_key_exists('faculty', $data) && array_key_exists('record', $data)) {
                ...
                if(!empty($this->comments)) {
                    ...
                    $inserted = PeerResponseModel::create($data['faculty']);
                    ...
                    PeerResponseItemModel::insert($response_items);
    ...
    public function get_questionnaires() {
        $questionnaire = PeerQuestionnaireModel::with('school_year', 'questionnaire_item.criteria')->where(function($query) {
        ...
        if($questionnaire->count() > 0) {
            ...
            return redirect()->route('faculty.department', ['evaluate' => $this->evaluate, 'semester' => $this->semester])->with('error', 'No questionnaires created yet');
        }
    ...
    public function is_responded($step) {
        ...
        $response_items = PeerResponseModel::with('items')->where('user_id', $user_id)
        ...
        if($response_items->exists()) {
            ...
            $reference = 'au_afears_peer_response_'.$data->id;
    ...
    public function go_back() {
        ...
        if (count($data) > 1 && !$data['faculty']['is_preview']) {
            $this->dispatch('leaving', ['has_saved' => true, 'route' => route('faculty.peer-faculty', ['evaluate' => $this->evaluate, 'semester' => $this->semester])]);
            $this->get_questionnaires();
        } else {
            return redirect()->route('faculty.peer-faculty', ['evaluate' => $this->evaluate, 'semester' => $this->semester]);
        }
    }
    ...
    public function mount(Request $request) {
        ...
        if($validate->fails()) {
            return redirect()->route('faculty.dashboard2');
        }
    ...
    public function render() {
        return view('livewire.faculty.evaluate');
    }
}
