...
class EvaluationResults extends Component
{
    ...
    public function result_view() {
        ...
        if($action == 'view') {
            ...
            //vader sentiment    
                // $comment_analysis = $this->analyzesentiment($response['comment']);
                ...
                $comments[] = [
                    ...
                    'neg' => $comment_analysis['neg'],
                    'neu' => $comment_analysis['neu'],
                    'pos' => $comment_analysis['pos'],
                    'compound' => $comment_analysis['compound'],
                    'sentiment' => $sentiment
                ];
                //add to array
                $vader_scores['compound'][] = floatval(number_format($comment_analysis['compound'], 2, '.', ''));
                $vader_scores['pos'][] = floatval(number_format($comment_analysis['pos'] * 100, 2, '.', ''));
                $vader_scores['neu'][] = floatval(number_format($comment_analysis['neu'] * 100, 2, '.', ''));
                $vader_scores['neg'][] = floatval(number_format($comment_analysis['neg'] * 100, 2, '.', ''));
